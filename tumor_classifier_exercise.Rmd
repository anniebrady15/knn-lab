...
data_subset$Class <- as.character(data_subset$Class)
...
*Answer*: To make sure we are having different values from training to testing
...
predictions <- predict(knn_model, newdata = testing_set)
predictions
# Confusion matrix  
confusionMatrix(predictions, testing_set$Class)

```

*Question*: What insights can you gather from the confusion matrix? Is the model performing well?

*Answer*: bit shit. 73% accuracy. 
...
special_k <- knn_model$bestTune
print(special_k)
```

*Question*: What is the optimal value of 'k'? How would you explain its significance?

*Answer*: 5. Optimal number of points used which guesses correctly
